...
namespace HRMS
{
    public partial class EditMultipleClaim : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ...
                int claimID = int.Parse(Session["claimID"].ToString());
                ...
                ds1 = aGroupClaim.getClaimDetails(claimID);

                for (int i = 0; i < ds1.Tables[0].Rows.Count; i++)
                {
                    dr = dt.NewRow();
                    dr["claimType"] = ds1.Tables[0].Rows[i]["claimType"].ToString();
                    dr["description"] = ds1.Tables[0].Rows[i]["description"].ToString();
                    dr["amt"] = ds1.Tables[0].Rows[i]["amount"].ToString();
                    dr["attachment"] = ds1.Tables[0].Rows[i]["attachment"].ToString();
                   


                    /*IDictionary<int, HttpPostedFile> test = (Dictionary<int, HttpPostedFile>)Session["filepath"];

                    test[i] = (HttpPostedFile) ds1.Tables[0].Rows[i]["attachment"];
                ...
                txtGroupTitle.Text = aGroupClaim.GetGTitle(claimID);
                txtDateFrom.Text = aGroupClaim.GetDateFrom(claimID);
                txtDateTo.Text = aGroupClaim.GetDateTo(claimID);
            ...
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dtCurrentTable.Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dtCurrentTable.Rows.Count; i++)
                    {
                        ...
                        DropDownList claimType = (DropDownList)gvMultiClaim.Rows[i].Cells[0].FindControl("ddlCliamType");
                        dtCurrentTable.Rows[i]["claimType"] = claimType.SelectedItem.ToString().Trim();
            ...
            if (ViewState["CurrentTable"] != null)
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    if (eVal == 0)
                    {
                        ...
                        int claimID = int.Parse(Session["claimID"].ToString());
                        mEClaim -= aGroupClaim.GetMEClaimAmount(claimID);
                        mClaim -= aGroupClaim.GetMClaimAmount(claimID);
                        pAClaim -= aGroupClaim.GetPAClaimAmount(claimID);
                        tClaim -= aGroupClaim.GetTClaimAmount(claimID);
                    }
                    else
                    {
            ...
            if (eVal == 0)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (sb1.ToString().Contains("- Meal Expenses"))
                            {

                            }
                            else
                            {
                                sb1.AppendLine("- Meal Expenses");
                            }
                        }
                        if (mClaim >= float.Parse(lblMedical.Text))
                        {
                            if (sb1.ToString().Contains("- Medical"))
                            {

                            }
                            else
                            {
                                sb1.AppendLine("- Medical");
                            }
                        }
                        if (pAClaim >= float.Parse(lblPA.Text))
                        {
                            if (sb1.ToString().Contains("- Phone Allowance"))
                            {

                            }
                            else
                            {
                                sb1.AppendLine("- Phone Allowance");
                            }
                        }
                        if (tClaim >= float.Parse(lblTransport.Text))
                        {
                            if (sb1.ToString().Contains("- Transport"))
                            {
                            }
                            else
                            {
                                sb1.AppendLine("- Transport");
                            }
                        }

                        lblErrorMessage.Text = sb1.ToString().Replace(Environment.NewLine, "<br />");
        ...
        protected void btnConfirm_Click(object sender, EventArgs e)
        {
            ...
            IDictionary<int, HttpPostedFile> test = (Dictionary<int, HttpPostedFile>)Session["filepath"];
            int result = 0;
            //GroupClaim c = new GroupClaim(claimId, dateFrom, dateTo);
            result = aGroupClaim.UpdateClaim(claimId, dateFrom, dateTo);
            if (result > 0)
            {
                //delete from db
                int result1 = 0;

                result1 = aGroupClaim.claimDelete(claimId);
                if (result1 > 0)
                {
                    int result2 = 0;
                    for (int i = 0; i < gv_Details.Rows.Count; i++)
                    {
                        ...
                        Label lblattachment = (Label)gv_Details.Rows[i].Cells[0].FindControl("lblAtt");
                        string claimT = claimType.Text;
                        string des = description.Text;
                        float amount = float.Parse(amt.Text);
                        string attachmentName = lblattachment.Text;
                        ...
                        if (test.ContainsKey(i))
                        {
                            ...
                            string filename = @"GroupClaim\" + file.FileName;
                                //test[i].SaveToFile(String.Format('ClaimDocument\' + test[i]), FileFormat.Docx);

                            file.SaveAs(savePath);        // file path where you want to upload

                                //string claimType, string title, string description, string attachment, float amount
                            GroupClaim g = new GroupClaim(claimId, claimT, groupTitle, des, file.FileName, amount);
                            result2 = g.groupClaimInsert();

                        }
                        else
                        {
                            GroupClaim g = new GroupClaim(claimId, claimT, groupTitle, des, attachmentName, amount);
                            result2 = g.groupClaimInsert();
                        }
        ...
        protected void lblAttOnClick(object sender, EventArgs e)
        {
            ...
            else if (test.ContainsKey(rowIndex))
            {
            ...
            {
                OpenNewBrowserWindow("\\GroupClaim\\\\" + attbtn.Text, this);
            }
